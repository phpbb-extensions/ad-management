...
class admin_input_test extends \phpbb_database_test_case
{
	...
	{
		return array(
			array(false, ['Ad Name #1', 'Ad Note #1', 'Ad Code #1', '', '', '', '5', '0', '', '', '', []], 0, ['FORM_INVALID']),
			array(true, ['', 'Ad Note #1', 'Ad Code #1', '', '', '', '5', '0', '', '', '', []], 0, ['AD_NAME_REQUIRED']),
			array(true, [str_repeat('a', 256), 'Ad Note #1', 'Ad Code #1', '', '', '', '5', '0', '', '', '', []], 0, ['AD_NAME_TOO_LONG']),
			array(true, ['Ad Name #1', 'Ad Note #1', 'Ad Code with emoji 😀', '', '', '', '5', '0', '', '', '', []], 0, ['AD_CODE_ILLEGAL_CHARS']),
			array(true, ['Ad Name #1', 'Ad Note #1', 'Ad Code #1', '', '', 'blah', '5', '0', '', '', '', []], 0, ['AD_END_DATE_INVALID']),
			array(true, ['Ad Name #1', 'Ad Note #1', 'Ad Code #1', '', '', '1970-01-01', '5', '0', '', '', '', []], 0, ['AD_END_DATE_INVALID']),
			array(true, ['Ad Name #1', 'Ad Note #1', 'Ad Code #1', '', '', '', '0', '0', '', '', '', []], 0, ['AD_PRIORITY_INVALID']),
			array(true, ['Ad Name #1', 'Ad Note #1', 'Ad Code #1', '', '', '', '11', '0', '', '', '', []], 0, ['AD_PRIORITY_INVALID']),
			...
			array(true, ['Ad Name #1', 'Ad Note #1', 'Ad Code #1', '1', array('above_header', 'above_footer'), '2033-01-01', '4', '1', '50', '30', 'admin', ['5']], 2, []),
		);
	}
	...
	{
		list($ad_name, $ad_note, $ad_code, $ad_enabled, $ad_locations, $ad_end_date, $ad_priority, $ad_content_only, $ad_views_limit, $ad_clicks_limit, $ad_owner, $ad_groups) = $data;

		self::$valid_form = $valid_form;
		$input_controller = $this->get_input_controller();

		$this->request->expects($this->exactly(12))
			->method('variable')
			->will($this->onConsecutiveCalls($ad_name, $ad_note, $ad_code, $ad_enabled, $ad_locations, $ad_end_date, $ad_priority, $ad_content_only, $ad_views_limit, $ad_clicks_limit, $ad_owner, $ad_groups));
